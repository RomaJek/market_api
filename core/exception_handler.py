...
""" DRF error shigip qalsa tusinikli standart JSON formatinda qaytariw ushin qayta jazildi """
...
def custom_exception_handler(exc, context):
    ...
    if isinstance(errors, list):    # errors list bolip kelgende isleydi ham nolinshi elementin shigarip beredi
        first_error = str(errors[0]) if errors else 'An error occurred.'  # 'An error occurred.' <- Xate juz berdi. (payda boldi)
        response.data = {
            'success': False,
            'data': None,
            'error': {
